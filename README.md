...
of [jamesrochabrun/SwiftOpenAI](https://github.com/jamesrochabrun/SwiftOpenAI),
based on upstream `4.4.7`. It is maintained independently and does not
automatically track subsequent upstream changes.

The `SwiftOpenAI` module and product names remain unchanged. This fork is used
by rootshell but remains an independently usable Swift package. It is not an
official upstream project and is not supported by the upstream maintainer.

## Fork changes

- Makes Responses API models tolerant of partial and provider-specific payloads.
- Adds the Responses extensions needed by the Codex backend, including encrypted
  reasoning replay and additional reasoning, streaming, and text options.
- Preserves raw API error payloads for callers that need provider-specific fields.
- Supports endpoints that require no authorization header, including local model
  servers such as Ollama, LM Studio, oMLX, and llama.cpp.
...
Add the public rootshell fork and pin the release exactly:

```swift
.package(
  url: "https://github.com/kitknox/SwiftOpenAI-rootshell.git",
  exact: "4.4.8"
)
```

In Xcode, choose `File` → `Add Package Dependencies`, enter the same HTTPS URL,
and select the exact `4.4.8` version.
