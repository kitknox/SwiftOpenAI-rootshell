...
public enum Authorization {
  case apiKey(String)
  case bearer(String)
  /// Send no credential at all. Local inference servers (Ollama, LM Studio, oMLX, llama.cpp)
  /// commonly run unauthenticated, and some reject a present-but-unknown token rather than
  /// ignoring it, so an empty `Bearer ` is not a substitute for omitting the header.
  case none
  ...
  /// `nil` when no header should be sent.
...
extension OpenAIService {
    ...
    async throws -> [[String: Any]]
  {
    ...
    guard response.statusCode == 200 else {
      ...
      do {
        ...
        // If decoding fails, include raw response body for debugging
        if let rawBody = String(data: data, encoding: .utf8), !rawBody.isEmpty {
          errorMessage = "status code \(response.statusCode): \(rawBody)"
        }
      }
    ...
    async throws -> AsyncThrowingStream<T, Error>
  {
    ...
    guard response.statusCode == 200 else {
      ...
      // The raw body is kept verbatim: callers map errors by inspecting fields
      // (code, plan_type, resets_at) that a decoded OpenAIErrorResponse would drop.
      if let (errorData, _) = try? await httpClient.data(for: httpRequest),
         let rawBody = String(data: errorData, encoding: .utf8), !rawBody.isEmpty
      {
        errorMessage = "status code \(response.statusCode): \(rawBody)"
      }
      throw APIError.responseUnsuccessful(
