...
public enum InputItem: Codable {
  ...
  /// Reasoning item replayed from a previous response (encrypted content round-trip)
  case reasoning(ReasoningInputItem)
  ...
  public init(from decoder: Decoder) throws {
    ...
    case "reasoning":
      self = try .reasoning(ReasoningInputItem(from: decoder))
...
// MARK: - ReasoningInputItem

/// A reasoning item replayed into the input of a follow-up request.
/// Used with `store: false` + `include: ["reasoning.encrypted_content"]`, where the
/// server returns encrypted reasoning that must accompany the function calls it
/// produced when the transcript is re-sent. No `id` is encoded — replay works on
/// `encrypted_content` alone.
public struct ReasoningInputItem: Codable {
  public init(summary: [SummaryText] = [], encryptedContent: String? = nil) {
    self.summary = summary
    self.encryptedContent = encryptedContent
  }

  public struct SummaryText: Codable {
  ...
  /// Reasoning summary parts, possibly empty
  public let summary: [SummaryText]

  /// Opaque encrypted reasoning content from a prior response
